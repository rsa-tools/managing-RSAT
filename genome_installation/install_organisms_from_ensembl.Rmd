---
title: "Install organisms from Ensembl"
...
# Installing genomes and variations from 

[\[sect:install\_ensembl\_genome\]](#sect:install_ensembl_genome)

includes a series of programs to download and install genomes from
Ensembl.
...
grep -v '^;' available_species_ensembl.tab | wc -l
...
awk -F'\t' '$2 ~ "variations"' available_species_ensembl.tab | grep -v '^;'
```
...
A more recent project called extends the project to a wider taxonomic range (in Oct 2014, there are \>15,000 genomes available at EnsemblGenomes, where as Ensembl only provides 69 genomes).

The program supports the installation of genomes from as well as . By
default, it opens a connection to the historical database, but the
option enables to install genomes from the new project .
...
You can then identify your genome of interest in the file , and start
the installation (don’t forget the option .
...
The program downloads variations from the Web site, and installs it on
the local site.

This program relies on , which must be installed beforehand on your
computer.
...
(there is one separate file per chromosome or contig).
