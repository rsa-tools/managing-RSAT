...
|---------|--------------------------|
|       25626   | total |
|       16735   | available_bacteria_2017-12-12.txt     |
|       7384    | available_viral_2017-12-12.txt        |
|       531     | available_archaea_2017-12-12.txt      |
|       245     | available_fungi_2017-12-12.txt        |
|       155     | available_invertebrate_2017-12-12.txt |
|       124     | available_vertebrate_other_2017-12-12.txt     |
|       109     | available_vertebrate_mammalian_2017-12-12.txt |
|       99      | available_plant_2017-12-12.txt        |
|       79      | available_protozoa_2017-12-12.txt     |
|       0       | available_plastid_2017-12-12.txt      |
|       0       | available_plasmid_2017-12-12.txt      |
|       0       | available_mitochondrion_2017-12-12.txt        |
...
1. The option `-species` requires to specify the Genus (with a leading uppercase) and species (in lowercases) separated by an undescore character (`_`), with exactly the same spelling and cases as on the NCBI FTP refseq site. 
...
ls -1 $RSAT/downloads/refseq/bacteria/Rhodospirillum_centenum/latest_assembly_versions
```
...
install-organism -v 2 -group bacteria \
  -species Rhodospirillum_centenum \
  -strain GCF_000016185.1_ASM1618v1 \
  -task parse
```
...
ls -l ${RSAT}/public_html/data/genomes/Rhodospirillum_centenum_GCF_000016185.1_ASM1618v1/genome/

## Measure the disk space occupied by this genome (note: this will be increased in the subsequent steps)
du -sm ${RSAT}/public_html/data/genomes/Rhodospirillum_centenum_GCF_000016185.1_ASM1618v1/genome/
```
...
retrieve-seq -all -org Rhodospirillum_centenum_GCF_000016185.1_ASM1618v1  -type downstream -from -1 -to -3 -o stop_seq.fasta

more stop_seq.fasta

## Check that most genes have ATG as start codon
oligo-analysis -v 1 -i stop_seq.fasta  -l 3 -1str -return occ,freq -sort | more
...
NCBI provides a summary file with indications of the sequencing status for each genome project: 
...
ASSEMBLY_URL=ftp://ftp.ncbi.nlm.nih.gov/genomes/refseq/bacteria/assembly_summary.txt
ASSEMBLY=ncbi_organisms_assembly_summary_bacteria_refseq_${TODAY}.txt
COMPLETE=ncbi_organisms_assembly_summary_bacteria_complete_${TODAY}.txt
ONECHROM=ncbi_organisms_assembly_summary_bacteria_chromosome_${TODAY}.txt
ONECHROM_AND_COMPLETE=chromosome_and_complete_bacterial_genomes_${TODAY}.txt
...
## Merge summary tables for bacterial species with
## at least one sequenced chromosome
cat ${COMPLETE} ${ONECHROM} >  ${ONECHROM_AND_COMPLETE}

## Count the number of selected species
wc -l ${COMPLETE} ${ONECHROM} ${ONECHROM_AND_COMPLETE}
...
## Number of reference or representative genomes

## For all genomes
grep -v '^#' ${ASSEMBLY} | cut -f 5 | sort | uniq -c | sort -n

## For those with at least one complete chromosome
grep -v '^#' ${ONECHROM_AND_COMPLETE} | cut -f 5 | sort | uniq -c | sort -n
```
...
- This tan take a considerable time dependingon the number of selected species and bandwidth of your computer. 
...
SPECIES_TO_INSTALL=bacteria_to_install.txt

## Get the two first words of the 8th column as Genus and species names
cut -f 8 ${ONECHROM_AND_COMPLETE} \
  | awk '{print $1"_"$2}' \
   > ${SPECIES_TO_INSTALL}
