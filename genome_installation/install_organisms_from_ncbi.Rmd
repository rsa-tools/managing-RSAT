...
grep -v '^#' $RSAT/downloads/refseq/${GROUP}/assembly_summary.txt | wc -l
```
...
wc -l $RSAT/downloads/refseq/*/assembly_summary.txt |  sort -nr  | awk '{print "|\t"$1-15"\t| "$2"\t|"}' 
...
|---------|---------------------------|
|	349417	| $RSAT/downloads/refseq/bacteria/assembly_summary.txt	|
|	14979	| $RSAT/downloads/refseq/viral/assembly_summary.txt	|
|	2010	| $RSAT/downloads/refseq/archaea/assembly_summary.txt	|
|	569	| $RSAT/downloads/refseq/fungi/assembly_summary.txt	|
|	384	| $RSAT/downloads/refseq/invertebrate/assembly_summary.txt	|
|	383	| $RSAT/downloads/refseq/vertebrate_other/assembly_summary.txt	|
|	216	| $RSAT/downloads/refseq/vertebrate_mammalian/assembly_summary.txt	|
|	169	| $RSAT/downloads/refseq/plant/assembly_summary.txt	|
|	83	| $RSAT/downloads/refseq/protozoa/assembly_summary.txt	|
|	*368330*	| *total*	|
...
We can filter the assemblies in order to select a reasonable number of genomes to install. 

Firstly, we could select the so called reference genomes
...
The result can be printed in 3 formats : 
...
install-organism -v 1 -group bacteria  -task list \
  -list_format table \
  -filter  refseq_category "reference genome"  -o reference_genomes.tsv
```

- *bash*  a bash script sith the commands to install each of the selected assemblies

```{bash eval=FALSE}
install-organism -v 1 -group bacteria  -task list \
  -list_format bash \
  -filter  refseq_category "reference genome" -o reference_genomes.bash
```


### Listing the available assemblies for a species of interest

The option `-species` enabled to select a subset of assemblies corresponding to a given species of a given genus. THe option should be followed by a string concatenating the Genus (with a leading uppercase) and species (in lowercases) separated by an undescore character (`_`), with exactly the same spelling and cases as on the NCBI FTP refseq site. 
...
# export SPECIES=Mycobacterium_tuberculosis

## List the assemblies for a given species 
...
With *Escherichia_coli* as species, we get 34417 assemblies (24/04/2024).


### Listing the available assemblies - representative and complete genomes

We can combine several filtering criteria to select a more reasonable number of assemblies. 
For example, we could select the assemblies labeled as "representative genome" in the column refseq_category, and "Complete Genome" in the colum ""
...
install-organism -v 1 -group bacteria  -task list \
  ...
  -o representative_complete_major.tsv
```
...
```{bash eval=FALSE}
grep -v '^;' representative_complete_major.tsv | grep -v '^#' | cut -f 6 | sort  | uniq -c|sort -nr | less
```

We can export a bash script with the instructions to install each of these genomes. 
...
install-organism -v 1 -group bacteria  -task list \
  ...
  -filter refseq_category "representative genome" \
  -filter assembly_level "Complete Genome" \
  -o representative_complete_major.bash
...
We will now download the genome of a selected assembly, and install it on our RSAT server. As an illustration, we will install the reference strain K-12 of *Escherichia coli*. 
...
ASSEMBLY_URL=https://ftp.ncbi.nlm.nih.gov/genomes/refseq/${GROUP}/assembly_summary.txt
